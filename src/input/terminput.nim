## Terminal Input Backend for tStorie
##
## Handles ANSI/CSI escape sequence parsing for terminal input.
## Provides keyboard, mouse, and resize event detection.
...
proc handleCsi(vt: var TerminalInputParser, command: char): seq[InputEvent] =
  ...
  proc parseModsAndAction(vt: TerminalInputParser): (set[uint8], InputAction) =
    ...
    # Check if Alt was pressed via ESC prefix (prevEsc)
    if vt.prevEsc:
      result[0].incl ModAlt
    ...
    if vt.state == CSIIntermed:
      if isIntermed(c):
        ...
        for event in vt.handleCsi(c):
          result.add event
        vt.prevEsc = false  # Reset after CSI sequence uses it
      vt.state = Normal
      continue
      ...
      if c1Allowed and c.int >= 0x80 and c.int < 0xa0:
        ...
        if k == text.len:
          if k < n:
            ...
            break
          # Capture modifiers (Alt via prevEsc)
          var textMods: set[uint8] = {}
          if vt.prevEsc:
            textMods.incl ModAlt
            vt.prevEsc = false
          
          if vt.utf8Buffer.len > 0:
            result.add InputEvent(kind: TextEvent, text: vt.utf8Buffer & text[i..<k].join(""), textMods: textMods)
            vt.utf8Buffer.setLen(0)
          else:
            result.add InputEvent(kind: TextEvent, text: text[i..<k].join(""), textMods: textMods)
          vt.inUtf8 = false
          vt.utf8Remaining = 0
        else:
          # Capture modifiers (Alt via prevEsc)
