...
proc generateNimProgram*(doc: MarkdownDocument, filename: string = "untitled.md"): string =
  ...
  result &= "var gBgLayer, gFgLayer: Layer  # Layer references for drawing API\n"
  result &= "var gRunning {.global.} = true\n"
  result &= "when not defined(emscripten):\n"
  result &= "  var gTerminalState: TerminalState\n"
  result &= "\n"
  
  # Drawing API wrappers
  result &= "# Drawing API - convenience wrappers using layer references\n"
  result &= "proc bgClear() = gBgLayer.buffer.clear()\n"
  result &= "proc bgClearTransparent() = gBgLayer.buffer.clearTransparent()\n"
  result &= "proc bgWrite(x, y: int, ch: string, style: Style) = gBgLayer.buffer.write(x, y, ch, style)\n"
  result &= "proc bgWriteText(x, y: int, text: string, style: Style) = gBgLayer.buffer.writeText(x, y, text, style)\n"
  result &= "proc bgFillRect(x, y, w, h: int, ch: string, style: Style) = gBgLayer.buffer.fillRect(x, y, w, h, ch, style)\n"
  result &= "proc fgClear() = gFgLayer.buffer.clear()\n"
  result &= "proc fgClearTransparent() = gFgLayer.buffer.clearTransparent()\n"
  result &= "proc fgWrite(x, y: int, ch: string, style: Style) = gFgLayer.buffer.write(x, y, ch, style)\n"
  result &= "proc fgWriteText(x, y: int, text: string, style: Style) = gFgLayer.buffer.writeText(x, y, text, style)\n"
  result &= "proc fgFillRect(x, y, w, h: int, ch: string, style: Style) = gFgLayer.buffer.fillRect(x, y, w, h, ch, style)\n"
  result &= "\n"
  ...
  result &= "    \n"
  result &= "    # Initialize standard layers\n"
  result &= "    gBgLayer = newLayer(\"background\", w, h, 0)\n"
  result &= "    gFgLayer = newLayer(\"foreground\", w, h, 1)\n"
  result &= "    gState.layers.add(gBgLayer)\n"
  result &= "    gState.layers.add(gFgLayer)\n"
  result &= "    \n"
...
proc generateTStorieIntegratedProgram*(doc: MarkdownDocument, filename: string = "untitled.md"): string =
  ...
  # Drawing API wrappers (match runtime nimini API)
  result &= "# Drawing API - convenience wrappers using layer references\n"
  result &= "proc bgClear() = gBgLayer.buffer.clear()\n"
  result &= "proc bgClearTransparent() = gBgLayer.buffer.clearTransparent()\n"
  result &= "proc bgWrite(x, y: int, ch: string, style: Style) = gBgLayer.buffer.write(x, y, ch, style)\n"
...
proc exportToTStorieNimOptimized*(doc: MarkdownDocument, filename: string = "untitled.md"): tuple[code: string, stats: OptimizationStats] =
  ...
  result.code &= "var gBgLayer, gFgLayer: Layer  # Layer references for drawing API\n"
  ...
  # Drawing API wrappers (match runtime nimini API)
  result.code &= "# Drawing API - convenience wrappers using layer references\n"
  result.code &= "proc bgClear() = gBgLayer.buffer.clear()\n"
  result.code &= "proc bgClearTransparent() = gBgLayer.buffer.clearTransparent()\n"
  result.code &= "proc bgWrite(x, y: int, ch: string, style: Style) = gBgLayer.buffer.write(x, y, ch, style)\n"
  result.code &= "proc bgWriteText(x, y: int, text: string, style: Style) = gBgLayer.buffer.writeText(x, y, text, style)\n"
  result.code &= "proc bgFillRect(x, y, w, h: int, ch: string, style: Style) = gBgLayer.buffer.fillRect(x, y, w, h, ch, style)\n"
  result.code &= "proc fgClear() = gFgLayer.buffer.clear()\n"
  result.code &= "proc fgClearTransparent() = gFgLayer.buffer.clearTransparent()\n"
  result.code &= "proc fgWrite(x, y: int, ch: string, style: Style) = gFgLayer.buffer.write(x, y, ch, style)\n"
  result.code &= "proc fgWriteText(x, y: int, text: string, style: Style) = gFgLayer.buffer.writeText(x, y, text, style)\n"
  result.code &= "proc fgFillRect(x, y, w, h: int, ch: string, style: Style) = gFgLayer.buffer.fillRect(x, y, w, h, ch, style)\n"
  result.code &= "\n"
  ...
  result.code &= "    \n"
  result.code &= "    # Initialize standard layers\n"
  result.code &= "    gBgLayer = newLayer(\"background\", w, h, 0)\n"
  result.code &= "    gFgLayer = newLayer(\"foreground\", w, h, 1)\n"
  result.code &= "    gState.layers.add(gBgLayer)\n"
  result.code &= "    gState.layers.add(gFgLayer)\n"
  result.code &= "    \n"
