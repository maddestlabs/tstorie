...
proc generateNimProgram*(doc: MarkdownDocument, filename: string = "untitled.md"): string =
  ...
  result &= "    \n"
  result &= "    # Setup signal handlers for graceful shutdown on CTRL-C\n"
  result &= "    setupSignalHandlers(proc(sig: cint) {.noconv.} = gRunning = false)\n"
  result &= "    \n"
  ...
  if ctx.updateCode.len > 0 or ctx.renderCode.len > 0:
    ...
    if ctx.inputCode.len > 0:
      ...
      for code in ctx.inputCode:
        ...
        result &= indented & "\n"
      result &= "\n"
      result &= "        # Check if mouse tracking should be re-enabled\n"
      result &= "        checkMouseTrackingReenabled(gInputParser)\n"
      result &= "\n"
...
proc exportToTStorieNimOptimized*(doc: MarkdownDocument, filename: string = "untitled.md"): tuple[code: string, stats: OptimizationStats] =
  ...
  result.code &= "    \n"
  result.code &= "    # Setup signal handlers for graceful shutdown on CTRL-C\n"
  result.code &= "    setupSignalHandlers(proc(sig: cint) {.noconv.} = gRunning = false)\n"
  result.code &= "    \n"
  ...
  if ctx.inputCode.len > 0:
    ...
    result.code &= "        \n"
    result.code &= "        # Check if mouse tracking should be re-enabled\n"
    result.code &= "        checkMouseTrackingReenabled(gInputParser)\n"
    result.code &= "        \n"
