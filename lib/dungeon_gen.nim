...
##
## NIMINI BINDINGS:
## Uses auto_pointer system for automatic binding generation.
## The DungeonGenerator is a ref object passed through nimini as integer pointer IDs.
## All exported methods are automatically wrapped by the auto_pointer macros.
##
## See dungeon_bindings.nim for registration.

import std/[random, sequtils]
import ../nimini/auto_pointer
import ../nimini/runtime  # For toInt, Value types
...
# Set up auto-pointer system for DungeonGenerator
...
proc newDungeonGenerator*(width, height: int, seed: int = 0): DungeonGenerator {.autoExposePointer.} =
...
proc update*(gen: DungeonGenerator): bool {.autoExposePointerMethod.} =
...
proc generate*(gen: DungeonGenerator) {.autoExposePointerMethod.} =
...
# ==============================================================================
# Accessor Methods (for auto-binding)
# ==============================================================================

proc getCellAt*(gen: DungeonGenerator, x, y: int): int {.autoExposePointerMethod.} =
  ## Get cell type at x, y coordinates (returns int ordinal)
  let cell = gen.getCell(vec2(x, y))
  return ord(cell)

proc getWidth*(gen: DungeonGenerator): int {.autoExposePointerMethod.} =
  ## Get dungeon width
  return gen.width

proc getHeight*(gen: DungeonGenerator): int {.autoExposePointerMethod.} =
  ## Get dungeon height
  return gen.height

proc getStep*(gen: DungeonGenerator): int {.autoExposePointerMethod.} =
  ## Get current generation step
  return gen.step

proc isStillGenerating*(gen: DungeonGenerator): bool {.autoExposePointerMethod.} =
  ## Check if dungeon is still generating
  return gen.isGenerating

# ==============================================================================
# Manual Bindings (special cases that don't fit auto_pointer pattern)
# ==============================================================================

proc nimini_dungeonGetCellChar*(env: ref Env; args: seq[Value]): Value =
...
export getCell, inBounds, getCellAt, getWidth, getHeight, getStep, isStillGenerating

# Explicit module initialization - ensures plugin registration happens
# The macros above generate register_* procs, we just need to queue them explicitly for WASM
proc initDungeonGenModule*() =
  ...
  # From autoPointer(DungeonGenerator):
  queuePluginRegistration(register_releaseDungeonGenerator)
  # From {.autoExposePointer.} and {.autoExposePointerMethod.} pragmas:
  queuePluginRegistration(register_newDungeonGenerator)
  queuePluginRegistration(register_update)
  queuePluginRegistration(register_generate)
  queuePluginRegistration(register_getCellAt)
  queuePluginRegistration(register_getWidth)
  queuePluginRegistration(register_getHeight)
  queuePluginRegistration(register_getStep)
  queuePluginRegistration(register_isStillGenerating)
  # Manual registration from below:
  queuePluginRegistration(proc() = registerNative("dungeonGetCellChar", nimini_dungeonGetCellChar, description = "Get display character for cell type"))
