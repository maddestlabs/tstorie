...
proc initTextureCache*(): TextureCache =
  ...
  result.maxCacheSize = 100  # Cache up to 100 text textures
...
proc renderText*(renderer: ptr SDL_Renderer, font: ptr TTF_Font, 
  ...
  if globalTextureCache.textures.len >= globalTextureCache.maxCacheSize:
    # Find oldest texture and remove it
    var oldestKey = ""
    var oldestFrame = frameCounter
    for key, cached in globalTextureCache.textures:
      if cached.lastUsed < oldestFrame:
        oldestFrame = cached.lastUsed
        oldestKey = key
    
    if oldestKey != "":
      SDL_DestroyTexture(globalTextureCache.textures[oldestKey].texture)
      globalTextureCache.textures.del(oldestKey)
