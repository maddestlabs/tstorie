...
nimini_newLabel("debug", 2, 22, 70, 1, "")
```
...
var debugMsg = "Event: " & event.type
if event.type == "key":
  debugMsg = debugMsg & " code=" & str(event.keyCode) & " action=" & event.action
elif event.type == "mouse":
  debugMsg = debugMsg & " x=" & str(event.x) & " y=" & str(event.y) & " btn=" & event.button & " action=" & event.action
nimini_widgetSetText("debug", debugMsg)
