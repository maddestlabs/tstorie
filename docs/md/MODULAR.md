...
- Unicode whitespace handling (U+2800 Braille blank, etc. render correctly as blank space)
...
- Front matter variable exposure (fixed: dynamic content loading now properly exposes front matter variables to Nimini environment)
- Dynamic font loading from front matter (Google Fonts support for CJK/Unicode fonts - optional, uses default 3270 font)
...
### Font System - Progressive Loading

The SDL3 build uses **progressive font loading** for fast startup with comprehensive glyph coverage:

**Phase 1: Immediate (33KB)**
- Starts with `iosevka-startup.ttf` bundled in WASM
- ASCII only for minimal file size
- App interactive in <1 second

**Phase 2: Background (1.4MB, after 2s)**
- Loads `iosevka-full.ttf` with full Unicode coverage
- Includes: 200+ languages, box drawing, mathematical symbols
- Hot-swaps font transparently after initial render

**Startup Performance:**
```
Time 0s:    WASM loads (~1.5MB) + Font data (33KB)
Time 0.5s:  App renders with minimal font (33KB subset)
Time 2.5s:  Full font loaded (1.4MB), Unicode support appears
```

**Font Coverage:**
- ✅ ASCII, Latin Extended, box drawing (immediate)
- ✅ Iosevka Fixed SS04 - Menlo-inspired, terminal-optimized
- ✅ 200+ languages, Cyrillic, Greek, Vietnamese (after progressive load)
- ✅ Mathematical symbols, arrows, geometric shapes
- ❌ Programming icons/Nerd Fonts (would need separate icon font)
- ❌ Emoji - would need Noto Color Emoji (~10MB)

**Note:** SDL3_ttf loads single TTF file without fallback (unlike browser CSS fonts).
...
### Front Matter Variables Not Exposed in Dynamically Loaded Content (RESOLVED)
**Root Cause:** The `setMarkdownContent()` function (called when loading external demos via `?content=` URL parameter) was updating the `storieCtx.frontMatter` dictionary but never calling `exposeFrontMatterVariables()` to expose those variables as globals in the Nimini environment.

**Pipeline:**
- **Initial load**: `initStorieContext()` → parses embedded index.md → calls `exposeFrontMatterVariables()` ✅
- **Dynamic load (broken)**: JS fetches demo.md → `setMarkdownContent()` → updates frontMatter → ❌ (never exposed)
- **Dynamic load (fixed)**: JS fetches demo.md → `setMarkdownContent()` → updates frontMatter → `exposeFrontMatterVariables()` → ✅

**Symptoms:**
- Demos loaded via `?content=minesweeper` would throw "Undefined variable 'chars'" error
- Front matter values like `chars`, `theme`, `fontsize` weren't accessible in on:init blocks
- Initial embedded content worked fine, only dynamic loading was broken

**Fix:**
- Added `exposeFrontMatterVariables()` call in `setMarkdownContent()` after updating front matter (line 1768 in tstorie.nim)
- Now front matter variables are properly exposed before on:init blocks execute

**Files Changed:**
- [tstorie.nim](tstorie.nim) - Added exposeFrontMatterVariables() call in setMarkdownContent()

**Status:** ✅ Fixed - front matter variables now properly exposed for dynamically loaded content

### Unicode Whitespace Characters Rendering as Visible Blocks (RESOLVED)
**Root Cause:** The SDL3 rendering code only checked for ASCII space `" "` before skipping character rendering. Unicode whitespace/invisible characters like U+2800 (⠀ Braille Pattern Blank) were being rendered to the glyph texture, and TTF fonts would render them as visible "tofu" replacement blocks instead of blank space.

**Comparison:**
- **Regular WASM build (WebGL)**: Renders U+2800 correctly as blank space (no visible glyph) ✅
- **SDL3 (broken)**: U+2800 renders as visible block character □ ❌
- **SDL3 (fixed)**: U+2800 skipped during rendering → displays as blank ✅

**Affected Characters:**
- U+2800 (⠀) - Braille Pattern Blank (commonly used for spacing in demos like intro.md, depths.md)
...
**Symptoms:**
- intro.md, her.md, depths.md all showed visible blocks where blank spacing should be
- Used for vertical spacing in markdown documents
- Made layout look broken with unwanted visible characters

**The Issue:**
In [backends/sdl3/sdl_canvas.nim](backends/sdl3/sdl_canvas.nim) renderCellsToPixels(), line ~518:
```nim
# WRONG: Only checks ASCII space
if cell.ch != " " and cell.ch.len > 0:
  # Render glyph...
```

This caused Unicode whitespace to be rendered to the glyph atlas, and most fonts render these as visible "missing glyph" blocks.

**Fix:**
Added explicit checks for common Unicode whitespace characters using UTF-8 byte sequences:
```nim
...
Now these characters are skipped during rendering, displaying as blank space (background color only).

**Files Changed:**
- [backends/sdl3/sdl_canvas.nim](backends/sdl3/sdl_canvas.nim) - Added Unicode whitespace detection in renderCellsToPixels()

**Testing:**
```bash
# Test intro demo with U+2800 characters
http://localhost:8000/index-modular.html?content=intro

# Test depths demo
http://localhost:8000/index-modular.html?content=depths
```

Look for proper spacing between sections - should not see visible block characters.

**Status:** ✅ Fixed - Unicode whitespace characters now render as blank space, not visible blocks

**Note on Unicode Coverage**: The default bundled font (3270-Regular.ttf) is a programming font with limited Unicode support. It covers Latin, Cyrillic, and common symbols but does NOT include:
- CJK characters (Chinese, Japanese, Korean) - e.g., 日本語
- Arabic script - e.g., العربية  
- Emoji - e.g., 😀 🎨 🚀
- Subscript/superscript numbers - e.g., ₁₂₃

These characters will render as "tofu" replacement boxes (□). 

**Solution**: Use the dynamic font loading feature to load a comprehensive Unicode font:
```yaml
---
font: "Noto+Sans"  # Good general Unicode coverage
# OR
font: "Noto+Sans+JP"  # Includes Japanese
---
```

The WebGL build doesn't have this limitation because it uses the browser's CSS font fallback system, which automatically uses system fonts when a character isn't available in the primary font.
