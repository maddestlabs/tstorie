...
# === CPU MAZE GENERATION ===
# Simple but effective hash - avoids overflow with small multipliers
proc hashCPU(val: int): int =
  var x = val
  if x < 0:
    x = -x
  
  # Use small multipliers and frequent modulo to prevent overflow
  # Multiple rounds with different primes for good distribution
  x = (x % 65536)
  x = ((x * 31) + 7919) % 65521
  x = ((x * 37) + 6971) % 65521
  x = ((x * 41) + 5953) % 65521
  ...
  # Mix in some division-based operations (simulates bit shifting)
  var high = x div 256
  var low = x % 256
  x = ((high * 59) + (low * 61) + 2003) % 65521
  
  # Expand to larger range
  x = ((x * 101) + (x div 10)) % 2147483647
  
  if x < 0:
    x = -x
  return x

proc generateMazeCPU(seed: int):
  echo("Generating maze on CPU with seed: " & str(seed))
  var startTime = getTime()
  
  # Binary Tree algorithm - same as GPU version
  var cellIdx = 0
  while cellIdx < mazeCellCount():
    var x = cellIdx % mazeWidth
    var y = cellIdx div mazeWidth
    
    var cellSeed = seed + cellIdx
    var rand = hashCPU(cellSeed) % 100
    
    var cellValue = 0
    
    # Binary tree: go north (50%) or east (50%)
    # Cell encoding: 1=north passage, 2=east passage, 3=both
    if y > 0 and (rand < 50 or x >= mazeWidth - 1):
      cellValue = 1  # North passage
    elif x < mazeWidth - 1:
      cellValue = 2  # East passage
    
    mazeCells[mazeMetaSize + cellIdx] = cellValue
    cellIdx = cellIdx + 1
  
  cpuGenTime = (getTime() - startTime) * 1000.0  # Convert to ms
  echo("CPU generation complete: " & str(cpuGenTime) & "ms")

proc smoothMazeCPU():
  echo("Smoothing maze on CPU")
  var startTime = getTime()
  ...
  while cellIdx < mazeCellCount():
    var x = cellIdx % mazeWidth
    var y = cellIdx div mazeWidth
    var inIndex = mazeMetaSize + cellIdx
    
    var cellValue = int(tempCells[inIndex])
    
    # Count open passages in neighbors
    var openCount = 0
    
    if x > 0:
      var left = int(tempCells[inIndex - 1])
      if left > 0:  # Has any passage
        openCount = openCount + 1
    if x < mazeWidth - 1:
      var right = int(tempCells[inIndex + 1])
      if right > 0:
        openCount = openCount + 1
    if y > 0:
      var top = int(tempCells[inIndex - mazeWidth])
      if top > 0:
        openCount = openCount + 1
    if y < mazeHeight - 1:
      var bottom = int(tempCells[inIndex + mazeWidth])
      if bottom > 0:
        openCount = openCount + 1
    
    # If isolated, add a north passage
    # Use arithmetic to add bit: if cellValue is 0 or 2, add 1
    if openCount <= 1 and x > 0 and y > 0:
      if cellValue == 0:
        cellValue = 1  # Add north passage
      elif cellValue == 2:
        cellValue = 3  # Add north to existing east
    
    mazeCells[inIndex] = cellValue
    cellIdx = cellIdx + 1
  
  var smoothTime = (getTime() - startTime) * 1000.0
  cpuGenTime = cpuGenTime + smoothTime
  echo("CPU smoothing complete: " & str(smoothTime) & "ms (total: " & str(cpuGenTime) & "ms)")
...
proc onMazeGenerated():
  ...
  if generationStep == 1 and smoothingEnabled:
    ...
    # Track mode-specific counts
    if lastGenMode == "GPU":
      gpuGenCount = gpuGenCount + 1
      gpuGenTime = (getTime() - lastGenTime) * 1000.0  # Convert to ms
      echo("GPU generation time: " & str(gpuGenTime) & "ms")
    else:
      cpuGenCount = cpuGenCount + 1
...
proc generateMaze():
  ...
  var availableH = termHeight - (if showDebugInfo: 12 else: 6)
  if availableW < 21:
    availableW = 21
  if availableH < 21:
    availableH = 21
  ...
  # Route to CPU or GPU based on mode
  if useCPU:
    lastGenMode = "CPU"
    echo("[CPU MODE] Generating maze with seed: " & str(seedArray[0]))
    generateMazeCPU(seedArray[0])
    
    if smoothingEnabled:
      generationStep = 2
      smoothMazeCPU()
    
    # CPU is synchronous, so directly finalize
    mazeReady = true
    generating = false
    generationStep = 0
    generationCount = generationCount + 1
    ...
    lastGenMode = "GPU"
    echo("[GPU MODE] Generating maze with seed: " & str(seedArray[0]))
...
if event.type == "text":
  if event.text == " ":
    ...
    if not generating:
      useCPU = not useCPU
      var mode = if useCPU: "CPU" else: "GPU"
...
# Generate first maze after a few frames (gives WebGPU time to initialize)
if not generating and not mazeReady and generationCount == 0:
  if getFrameCount() > 5:
    echo("[Init] Starting first maze generation...")
    generateMaze()
...
var modeIndicator = if useCPU: "[CPU]" else: "[GPU]"
var stats = modeIndicator & " Mazes: " & str(generationCount) & " | Player: (" & str(playerCellX) & "," & str(playerCellY) & ") | Explored: " & str(visitedCount) & "/" & str(mazeWidth * mazeHeight)
...
if showDebugInfo:
  var instrY = termHeight - 9
  draw(0, 1, instrY, "╔═══════════════════════════════════════════════════════════════════════════╗", getStyle("info"))
  draw(0, 1, instrY + 1, "║ 🎮 CONTROLS:  Arrow Keys = Navigate  |  SPACE = New Maze                ║", getStyle("info"))
  draw(0, 1, instrY + 2, "║ ⚙️  OPTIONS:   S = Smoothing  |  C = CPU/GPU Toggle  |  D = Debug      ║", getStyle("info"))
  draw(0, 1, instrY + 3, "║ 🧠 ALGORITHM: Binary Tree + Cellular Automata Smoothing                ║", getStyle("info"))
  draw(0, 1, instrY + 4, "╟───────────────────────────────────────────────────────────────────────────╢", getStyle("info"))
  
  # Performance comparison
  var perfLine1 = "║ ⚡ GPU: "
  if gpuGenCount > 0:
    perfLine1 = perfLine1 & str(gpuGenTime) & "ms (" & str(gpuGenCount) & " mazes)"
  else:
    perfLine1 = perfLine1 & "Not tested yet"
  while len(perfLine1) < 79:
    perfLine1 = perfLine1 & " "
  perfLine1 = perfLine1 & "║"
  draw(0, 1, instrY + 5, perfLine1, getStyle("info"))
  
  var perfLine2 = "║ 🖥️  CPU: "
  if cpuGenCount > 0:
    perfLine2 = perfLine2 & str(cpuGenTime) & "ms (" & str(cpuGenCount) & " mazes)"
  else:
    perfLine2 = perfLine2 & "Not tested yet"
  while len(perfLine2) < 79:
    perfLine2 = perfLine2 & " "
  perfLine2 = perfLine2 & "║"
  draw(0, 1, instrY + 6, perfLine2, getStyle("info"))
  
  var perfLine3 = "║ 📊 SPEEDUP: "
  if cpuGenCount > 0 and gpuGenCount > 0 and cpuGenTime > 0.0:
    var speedup = cpuGenTime / gpuGenTime
    perfLine3 = perfLine3 & str(int(speedup)) & "x faster with GPU!"
  else:
    perfLine3 = perfLine3 & "Generate mazes in both modes to compare"
  while len(perfLine3) < 79:
    perfLine3 = perfLine3 & " "
  perfLine3 = perfLine3 & "║"
  draw(0, 1, instrY + 7, perfLine3, getStyle("info"))
  
  draw(0, 1, instrY + 8, "╚═══════════════════════════════════════════════════════════════════════════╝", getStyle("info"))
else:
  draw(0, 1, termHeight - 3, "Press D for controls | C to toggle CPU/GPU | ESC to quit", getStyle("muted"))
